In this section, we'll prepare initiate a new chain with FireBFT consensus and prepare the initial Edge nodes.

## 1. Generate Keys

To initialize FireBFT consensus, we need to generate the necessary secrets for each node.

The `firechain firebft-secrets` command is used to generate account secrets for validators. The command initializes private keys for the consensus client (validators + networking) to a Secrets Manager config file.
...
| `--data-dir`    | The directory for the Firechain data if the local FS is used.                                          | `--data-dir /path/to/dir`  |
