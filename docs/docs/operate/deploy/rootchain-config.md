...
The `firechain` rootchain server command starts an ethereum/client-go container, which runs a new Geth node.
...
| `--data-dir`                 | The directory for the Firechain data if the local FS is used.              | `--data-dir test-chain-`                     |
...
To deploy the rootchain contracts, we use the `firechain rootchain deploy` command. Before deployment, consider the following actions:
...
You can also create a rootchain wallet and fund the nodes by using `firechain-cli`.
...
| `--data-dir`    | Directory for the Firechain data if the local FS is used                                            |         |
...
To fund the validators' accounts on the rootchain, we use the `firechain rootchain fund` command. When executed with the appropriate flags, it will:
...
Here's an example of how to fund a validator account (replace address value with your own):
  ...
  ```bash
  ./firechain rootchain fund --addresses 0x77C1eedFf656477462ce16084fE5Dc7F8a2507B9 --amounts 1000000000000000000
  ```
