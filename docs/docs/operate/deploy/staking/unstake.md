...
First, check your validator information by running the `firechain firebft validator-info` command.
...
| `--data-dir` | The directory for the Firechain data if the local FS is used | `/path/to/data/dir` |
...
```bash
./firechain firebft validator-info --data-dir ./test-chain-1
```
...
To unstake, use the `firechain firebft unstake` command.
...
| --data-dir          | Directory for the Firechain data if the local FS is used | --data-dir /path/to/data/dir          |
| --jsonrpc           | JSON-RPC interface                                         | --jsonrpc 0.0.0.0:8545                |

</details>

```bash
./firechain firebft unstake \
...
Once you have successfully unstaked, you will need to withdraw your unstaked tokens from the Edge-powered chain to the rootchain. To do so, use the `firechain firebft withdraw-child` command.
...
| `--data-dir` | The directory for the Firechain data if the local FS is used | `~/firechain/data` |
| `--jsonrpc` | The JSON-RPC interface (default "0.0.0.0:8545") | `127.0.0.1:8545` |

</details>

```bash
./firechain firebft withdraw-child \
    --account-dir <DATA_DIR> \
    --jsonrpc <JSONRPC_ADDR>
```
...
Once the exit event has been included in a checkpoint, you can send an exit transaction to execute the transaction on the rootchain. To do so, use the `firechain bridge exit` command.
...
Finally, you can withdraw your tokens from the rootchain to your address by using the `firechain firebft withdraw-root` command.
...
| --data-dir             | directory for the Firechain data     | --data-dir /path/to/data/dir       |
| --jsonrpc              | JSON-RPC interface                       | --jsonrpc 0.0.0.0:8545             |
...
| --to                   | address where to withdraw                | --to 0x456def                      |
  ...
  ```bash
  ./firechain firebft withdraw-root \
    --account-dir <DATA_DIR> \
